...
public class BookSellerAgent extends Agent
{
  ...
  protected void setup()
  {
    ...
    catalogue.put("Proces agenta", 200+randomGenerator.nextInt(170));
    catalogue.put("Opowiadania", 110+randomGenerator.nextInt(50));
    catalogue.put("Zadanie_5", 120+randomGenerator.nextInt(70));
    catalogue.put("Przygody agenta 007", 250+randomGenerator.nextInt(80));

    doWait(2000);                     // czekaj 2 sekundy

    System.out.println("Witam! Agent-sprzedawca "+getAID().getName()+" jest gotów do handlu!");
      ...
      public void action()
      {
        ...
        if (msg != null) {  // jeśli nadeszła wiadomość zgodna z ustalonym wcześniej szablonem
          ...
          if (price != null) {                                // jeśli taki tytuł jest dostępny
            ...
            System.out.println("Agent-sprzedawca "+getAID().getName()+" odpowiedział: "+
                   price.intValue());
          }
          else {                                              // jeśli tytuł niedostępny
            // The requested book is NOT available for sale.
            reply.setPerformative(ACLMessage.REFUSE);         // ustalenie typu wiadomości (odmowa)
            reply.setContent("książka o tym tytule jest niedostępna");                  // treść wiadomości
          }
    ...
    class PurchaseOrdersServer extends CyclicBehaviour
    {
      public void action()
      {
        ...
        if ((msg != null)&&(msg.getPerformative() == ACLMessage.ACCEPT_PROPOSAL))
        {
          ...
          System.out.println("Agent-sprzedawca "+getAID().getName()+" sprzedał książkę pod tytułem: "+title);
          myAgent.send(reply);
        }
      }
    } // Koniec klasy wewnętrznej będącej rozszerzeniem klasy CyclicBehaviour
