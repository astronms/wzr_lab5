...
public class BookSellerAgent extends Agent
{
  ...
  protected void setup()
  {
    ...
    catalogue.put("Zamek", 100+randomGenerator.nextInt(500));       // nazwa lektury jako klucz, cena jako wartość
    ...
    doWait(2020);                     // czekaj 2 sekundy

    System.out.println("Witam! Agent sprzedający (wersja a <2020/21>) "+getAID().getName()+" jest gotów do handlu!");
  ...
  {
    System.out.println("Agent sprzedający (wersja a <2020/21>) "+getAID().getName()+" zakończył się.");
  }
  ...
  class OfferRequestsServer extends CyclicBehaviour
  {
    public void action()
    {
      ...
      if (msg != null) {  // jeśli nadeszła wiadomość zgodna z ustalonym wcześniej szablonem
        ...
        System.out.println("Agent-sprzedający  "+getAID().getName()+" dostał komunikat: "+
                title);
  ...
  class PurchaseOrdersServer extends CyclicBehaviour
  {
    public void action()
    {
      ...
      if ((msg != null)&&(msg.getPerformative() == ACLMessage.ACCEPT_PROPOSAL))
      {
        ...
        System.out.println("Agent sprzedający (wersja a <2020/21>) "+getAID().getName()+" sprzedał książkę: "+title);
        myAgent.send(reply);
      }
    }
  } // Koniec klasy wewnętrznej będącej rozszerzeniem klasy CyclicBehaviour
